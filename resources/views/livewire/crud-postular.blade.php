    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="lg:ml-40 ml-10 space-x-8">
                    ...
                    @if($isOpen)
                        @include('livewire.postular-create')
                    @endif
            </div>
        </div>
            ...
            <table class="w-full divide-y divide-gray-200 table-auto">
              <thead class="bg-indigo-500 text-white">
                <tr class="text-left text-xs font-bold  uppercase">
                  ...
                  <td scope="col" class="px-6 py-3">CV</td>
                  <td scope="col" class="px-6 py-3">PUNTAJE</td>
                  <td scope="col" class="px-6 py-3">ESTADO</td>
                  <td scope="col" class="px-6 py-3">CODIGO EGRESADO</td>
                  <td scope="col" class="px-6 py-3">CODIGO TRABAJO</td>
                  <td scope="col" class="px-6 py-3">OPTIONS</td>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-200">
                @foreach ($postulars as $item)
                <tr class="text-sm font-medium text-gray-900">
                  ...
                  <td class="px-6 py-4">{{$item->ruta_cv}}</td>
                  <td class="px-6 py-4">{{$item->puntaje}}</td>
                  <td class="px-6 py-4">{{$item->estado}}</td>
                  <td class="px-6 py-4">{{$item->egresado_id}}</td>
                  <td class="px-6 py-4">{{$item->trabajo_id}}</td>
                  <td class="px-6 py-4">
            ...
            Swal.fire({
              ...
              confirmButtonText: 'Yes, delete it!'
                ...
                if (result.isConfirmed) {
                    //alert("del");
                    Livewire.emitTo('crud-postular','delete',id);
                    Swal.fire(
                        'Deleted!',
